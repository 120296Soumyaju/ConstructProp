...
const ProjectCard: React.FC<{ project: Project; onSelect: () => void }> = ({ project, onSelect }) => {
  return (
    <button onClick={onSelect} className="group relative overflow-hidden rounded-lg shadow-lg text-left w-full block">
      <img src={project.image} alt={project.title} className="w-full h-72 object-cover transform group-hover:scale-110 transition-transform duration-500" loading="lazy" decoding="async" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
      <div className="absolute bottom-0 left-0 p-6">
        <p className="text-amber-400 font-semibold text-sm mb-1">{project.category}</p>
...
const ProjectModal: React.FC<{ project: Project; onClose: () => void }> = ({ project, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto relative animate-fade-in-up"
      ...
      >
        ...
        <img src={project.image} alt={project.title} className="w-full h-64 md:h-80 object-cover rounded-t-lg" />
        <div className="p-6 md:p-8">
          <p className="text-amber-500 font-semibold mb-2">{project.category}</p>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProjects.map((project, index) => (
              <ProjectCard key={`${project.title}-${index}`} project={project} onSelect={() => setSelectedProject(project)} />
            ))}
          </div>
        </div>
      </section>
